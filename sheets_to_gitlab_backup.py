...
class SheetsToGitLab:
    ...
    def normalize_status(self, status):
        ...
        # Remove common formatting characters that might be added by spreadsheet alignment
    ...
    def create_gitlab_issue(self, record, gitlab_url, gitlab_token, project_id, default_assignee, default_milestone, default_label, project_mappings=None):
        ...
        try:
            ...
            # Add due date command from actual end date
            actual_end_date = record.get('END_DATE', '').strip()
            print(f"│   🔍 DEBUG - Due date processing:")
            print(f"│      END_DATE from record: '{actual_end_date}'")
            print(f"│      Record keys: {list(record.keys())}")
            
            if actual_end_date:
                try:
                    # Convert date format from DD-MM-YYYY to YYYY-MM-DD for GitLab
                    ...
                    gitlab_commands.append(f"/due {formatted_date}")
                    print(f"│   📅 Adding due date: {formatted_date} (from actual end date: {actual_end_date})")
                except Exception as e:
                    print(f"│   ⚠️ Error processing due date {actual_end_date}: {e}")
            else:
                print(f"│   ⚠️ No actual end date found in record, skipping due date")
            ...
            if milestone_to_use:
                milestone_value = milestone_to_use.replace('%', '')
                
                # Check if milestone_value is numeric (ID) or string (title)
                if milestone_value.isdigit():
                    # It's a milestone ID, use it directly
                    milestone_id = int(milestone_value)
                    issue_data['milestone_id'] = milestone_id
                    print(f"│   🎯 Adding to milestone ID {milestone_id}")
                else:
                    # It's a milestone title, look it up
                    try:
                        ...
                        if milestone_response.status_code == 200:
                            ...
                            if milestones:
                                ...
                                issue_data['milestone_id'] = milestone_id
                                print(f"│   🎯 Adding to milestone ID {milestone_id} (title: {milestone_value})")
                            else:
                                print(f"│   ⚠️ Milestone '{milestone_value}' not found, skipping milestone")
                        else:
                            print(f"│   ⚠️ Failed to lookup milestone '{milestone_value}' (Status: {milestone_response.status_code}), skipping milestone")
                    except Exception as e:
                        print(f"│   ⚠️ Error looking up milestone '{milestone_value}': {e}, skipping milestone")
            
            # Add assignee if specified
            if default_assignee:
                # Remove @ prefix and get the value
                assignee_value = default_assignee.replace('@', '')
                
                # Check if it's a numeric user ID or a username
                if assignee_value.isdigit():
                    # It's a user ID, use it directly
                    user_id = int(assignee_value)
                    issue_data['assignee_ids'] = [user_id]
                    print(f"│   👤 Assigning to user ID {user_id} (direct ID)")
                else:
                    # It's a username, look up user ID
                    try:
                        ...
                        if user_response.status_code == 200:
                            ...
                            if users:
                                user_id = users[0]['id']
                                issue_data['assignee_ids'] = [user_id]
                                print(f"│   👤 Assigning to user ID {user_id} (username: {assignee_value})")
                            else:
                                print(f"│   ⚠️ User '{assignee_value}' not found, skipping assignee")
                        else:
                            print(f"│   ⚠️ Failed to lookup user '{assignee_value}' (Status: {user_response.status_code}), skipping assignee")
                    except Exception as e:
                        print(f"│   ⚠️ Error looking up user '{assignee_value}': {e}, skipping assignee")
    ...
    def sync_sheets_to_gitlab(self):
        ...
        if enable_date_filter:
            filtered_records = []
            for record in records:
                git_id = record.get('GIT_ID', '').strip()
                
                # Only apply date filter to records without GIT_ID (new tasks to be created)
                if git_id:
                    # Keep records with GIT_ID regardless of date (they're existing tasks)
                ...
                if date_str:
                    try:
                        ...
                        # Skip records with invalid dates when date filter is enabled
                        print(f"│   ⏭️ Skipping row {record.get('_row_index', '?') + 2}: invalid date format")
                else:
                    # Skip records without dates when date filter is enabled
                    print(f"│   ⏭️ Skipping row {record.get('_row_index', '?') + 2}: no date provided")
        ...
        for row_index, record in enumerate(records):
            ...
            # Enhanced status processing to handle alignment and formatting issues
            raw_status = record.get('STATUS', '')
            status = raw_status.strip() if raw_status else ''
            
            # Debug status extraction with normalization
            if status:
                normalized_status = self.normalize_status(status)
                print(f"│   📋 Status extracted: '{raw_status}' -> '{status}' -> '{normalized_status}'")
                print(f"│   📋 Status length: {len(status)}, Normalized length: {len(normalized_status)}")
                has_whitespace = '\n' in status or '\r' in status or '\t' in status
                print(f"│   📋 Status contains whitespace: {has_whitespace}")
                print(f"│   📋 Status alignment check: '{repr(status)}'")
            ...
            if new_git_id:
                ...
                # Enhanced status processing to handle alignment and case issues
                status_processed = self.normalize_status(status)
                is_completion = self.is_completion_status(status)
                
                print(f"│   🔍 Status Analysis:")
                print(f"│      Original status: '{status}'")
                print(f"│      Normalized status: '{status_processed}'")
                print(f"│      Auto-close enabled: {enable_auto_close}")
                print(f"│      Is completion status: {is_completion}")
                print(f"│      Available completion statuses: {self.completion_statuses}")
                
                if enable_auto_close and is_completion:
