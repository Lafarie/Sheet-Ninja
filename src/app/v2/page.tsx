...
export default function SetupPage() {
  ...
  const handleTabChange = (tabId: string) => {
    setActiveTab(tabId);
    // Allow navigation to any tab - don't restrict based on current step
    const tabIndex = steps.findIndex((step) => step.id === tabId);
    if (tabIndex !== -1) {
  ...
  const handleSaveConfigSubmit = async (configData: { name: string; description: string; isDefault: boolean }) => {
    ...
    try {
      ...
      addNotification({
        ...
        message: `Configuration "${configData.name}" has been saved successfully`,
      });

      // Close modal and refresh the saved configs list
  ...
  const loadConfig = (savedConfig: any) => {
    try {
      // Safely update store with saved configuration
      useSetupStore.getState().loadFromSavedConfig(savedConfig);

      // Determine the appropriate step based on loaded configuration
      let targetStep = 1;
      let targetTab = "gitlab";

      // Check configuration completeness to determine current step
      if (savedConfig.gitlabUrl && savedConfig.gitlabToken) {
        targetStep = 2;
        targetTab = "sheets";
      }
      if (savedConfig.spreadsheetId && savedConfig.worksheetName) {
        targetStep = 3;
        targetTab = "columns";
      }
      if (savedConfig.columnMappings && Object.keys(savedConfig.columnMappings).length > 0) {
        targetStep = 4;
        targetTab = "projects";
      }
      if (savedConfig.projectMappings && savedConfig.projectMappings.length > 0) {
        targetStep = 5;
        targetTab = "filter";
      }
      if (savedConfig.columnMappings?.SELECTED_USER) {
        targetStep = 6;
        targetTab = "sync";
      } else if (savedConfig.projectMappings && savedConfig.projectMappings.length > 0) {
        // If no user filter but has project mappings, go to sync
        targetStep = 6;
        targetTab = "sync";
      }
      ...
      addNotification({
        ...
        message: `Loaded configuration: ${savedConfig.name}. Navigate to any step to continue setup.`,
      });
    } catch (error) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>Sign in to save your configurations and manage your sync setups</CardDescription>
          </CardHeader>
          <CardContent className="text-center space-y-4">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400">Your data is encrypted and secure</div>
          </CardContent>
        </Card>
      </div>
    );
        ...
        <div className="container mx-auto px-4 py-8">
          <Card className="mb-8 bg-gradient-to-r from-blue-600 to-purple-600 text-white border-0">
            <CardHeader className="text-center">
              ...
              <CardDescription className="text-blue-100 text-lg">Welcome back! Choose a configuration or create a new one</CardDescription>
            </CardHeader>
          </Card>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-gray-600 dark:text-gray-400">Your saved GitLab and Google Sheets sync configurations</CardDescription>
                </div>
              ...
              {loadingConfigs ? (
                ...
                <div className="space-y-4">
                  {savedConfigs.map((config) => (
                    <div key={config.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {!config.isDefault && (
                            <Button variant="ghost" size="sm" onClick={() => setAsDefault(config.id)} title="Set as default">
                              <Star className="w-4 h-4" />
                            </Button>
                          )}
                          <Button variant="ghost" size="sm" onClick={() => deleteConfig(config.id)} className="text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                        <div className="flex items-center gap-4">
                          <span>
                            <strong>GitLab:</strong> {config.gitlabUrl}
                          </span>
                          <span>•</span>
                          <span>
                            <strong>Sheet:</strong> {config.spreadsheetId}
                          </span>
                        </div>
                        <div className="flex items-center gap-4">
                          <span>
                            <strong>Projects:</strong> {config.projectMappings?.length || 0}
                          </span>
                          <span>•</span>
                          <span>
                            <strong>Updated:</strong> {new Date(config.updatedAt).toLocaleDateString()}
                          </span>
                        </div>
                      </div>
                    </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 relative">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="mb-8 bg-gradient-to-r from-blue-600 to-purple-600 text-white border-0">
          <CardHeader className="text-center">
            <div className="flex items-center justify-between">
              <Button variant="ghost" size="sm" onClick={() => setShowDashboard(true)} className="text-white hover:bg-white/10">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-6 gap-4">
                {steps.map((step, index) => {
                  ...
                  return (
                    <div key={step.id} className={`flex flex-col items-center p-3 rounded-lg transition-colors ${status === "active" ? "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800" : status === "completed" ? "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800" : "bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700"} border`}>
                      {getStepIcon(stepNumber)}
                      <span className={`text-sm font-medium mt-2 ${status === "active" ? "text-blue-900 dark:text-blue-100" : status === "completed" ? "text-green-900 dark:text-green-100" : "text-gray-500 dark:text-gray-400"}`}>{step.title}</span>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-2">
                    <Button onClick={handleSaveConfig} variant="outline" size="sm" className="w-full">
                      <Save className="w-4 h-4 mr-2" />
                      Save Configuration
                    </Button>
                    <Button
                    ...
                    <Button onClick={handleResetConfig} variant="outline" size="sm" className="w-full">
                      Reset Configuration
                    </Button>
                  </div>

                  <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                    <h4 className="font-medium mb-2 text-gray-900 dark:text-gray-100">Navigation</h4>
                    <div className="grid grid-cols-2 gap-1">
                      {steps.map((step, index) => {
                        const stepNumber = index + 1;
                        const Icon = step.icon;
                        return (
                          <Button key={step.id} onClick={() => handleTabChange(step.id)} variant={activeTab === step.id ? "default" : "outline"} size="sm" className="text-xs h-8">
